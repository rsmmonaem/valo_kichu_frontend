...
const DropshipperAdminDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "users" | "settings" | "security" | "pending" | "withdrawals"
  >("users");
  ...
  const [settings, setSettings] = useState({
    ...
    dropshipper_withdrawal_amount:500,
  });
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === "users") {
        ...
        const res = await authFetch("/admin/v1/dropshipping/withdrawals");
        const data = await res.json();
        if (res.ok) setWithdrawals(data.data);
  ...
  const toggleBan = async (id: number) => {
    try {
      const res = await authFetch(
        `/admin/v1/dropshipping/banned-ips/${id}/toggle`,
        { method: "POST" }
      );
      if (res.ok) fetchData();
    } catch (err) {
  ...
  const handleApprove = async (id: number) => {
    ...
    try {
      const res = await authFetch(
        `/admin/v1/dropshipping/users/${id}/approve`,
        {
          method: "POST",
        }
      );
  ...
  const handleToggleStatus = async (id: number) => {
    try {
      const res = await authFetch(
        `/admin/v1/dropshipping/users/${id}/toggle-status`,
        {
          method: "POST",
        }
      );
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        "Are you sure you want to delete this dropshipper? This action cannot be undone."
      )
    )
      return;
    try {
  ...
  const handleApproveWithdraw = async (id: number) => {
    const res = await authFetch(
      `/admin/v1/dropshipping/withdrawals/${id}/approve`,
      {
        method: "POST",
      }
    );
  ...
  const handleRejectWithdraw = async (id: number) => {
    const res = await authFetch(
      `/admin/v1/dropshipping/withdrawals/${id}/reject`,
      {
        method: "POST",
      }
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">
            Configure network margins, manage users and monitor security.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex bg-gray-100 p-1 rounded-2xl border border-gray-200 shadow-inner">
            ...
            >
              <span className="flex items-center gap-2">💸 Withdrawals</span>
            </button>
            <button
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 min-h-[600px] overflow-hidden">
        {isLoading ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {activeTab === "security" && (
              <SecurityList ips={bannedIps} toggleBan={toggleBan} />
            )}
            {activeTab === "withdrawals" && (
              <WithdrawalList
                ...
                <td className="px-8 py-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] font-black bg-gray-100 px-2 py-0.5 rounded uppercase text-gray-500 border border-gray-200 tracking-tighter mt-1 inline-block">
                  ...
                  {user.parent ? (
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 uppercase font-black">
                        {user.parent.role?.replace("_", " ")}
                      </p>
                    </div>
                  ) : (
                    <span className="text-xs font-black text-gray-300 uppercase italic">
                      Direct to Admin
                    </span>
                  )}
                </td>
...
const SettingsForm = ({ settings, setSettings, handleSave }: any) => (
  <div className="p-12 max-w-4xl mx-auto">
    <div className="mb-12">
      ...
      <p className="text-gray-500 font-medium">
        These percentages determine the default markup applied to the base
        purchase price for each level.
      </p>
    </div>
    ...
    <form onSubmit={handleSave} className="space-y-10">
      <div className="grid md:grid-cols-2 gap-12">
        <div className="space-y-4">
          <label className="block text-xs font-black text-gray-400 uppercase tracking-widest px-2">
            Master Dropshipper Margin (%)
          </label>
          <div className="relative group">
            ...
            <input
              ...
              onChange={(e) =>
                setSettings({
                  ...settings,
                  global_margin: parseFloat(e.target.value),
                })
              }
            />
          </div>
          ...
          <div className="relative group">
            ...
            <input
              ...
              onChange={(e) =>
                setSettings({
                  ...settings,
                  sub_dropshipper_margin: parseFloat(e.target.value),
                })
              }
            />
          </div>
        ...
        <div className="space-y-4">
          <label className="block text-xs font-black text-gray-400 uppercase tracking-widest px-2">
            Child Dropshipper Margin (%)
          </label>
          <div className="relative group">
            <TrendingUp
              size={24}
              className="absolute left-6 top-1/2 -translate-y-1/2 text-indigo-500"
            />
            <input
              type="number"
              ...
              onChange={(e) =>
                setSettings({
                  ...settings,
                  sub_sub_dropshipper_margin: parseFloat(e.target.value),
                })
              }
            />
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="relative group">
            ...
            <input
              type="number"
              className="w-full pl-16 pr-8 py-6 rounded-3xl bg-gray-50 border-2 border-transparent group-hover:bg-white group-hover:border-indigo-100 focus:bg-white focus:border-indigo-500 text-3xl font-black text-gray-900 transition-all outline-none"
              value={settings.dropshipper_withdrawal_amount}
              onChange={(e) =>
                setSettings({
                  ...settings,
                  dropshipper_withdrawal_amount: parseFloat(e.target.value),
                })
              }
            />
          </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm font-medium">
            Monitor suspicious activity and override automated bans.
          </p>
        </div>
      </div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {existingUsers
                  .filter(
                    (u: any) =>
                      (formData.role === "sub_dropshipper" &&
                        u.role === "dropshipper") ||
                      (formData.role === "sub_sub_dropshipper" &&
                        u.role === "sub_dropshipper")
                  )
                  .map((u: any) => (
...
const WithdrawalList = ({ withdrawals, onApprove, onReject }: any) => {
  return (
    <div className="p-6">
      <h2 className="text-xl font-black mb-6">Withdrawal Requests</h2>
          ...
          <tbody>
            {withdrawals.map((w: any) => (
              <tr key={w.id} className="border-t">
                ...
                <td className="px-6 py-4 text-xs">{w.transaction_id}</td>
                  ...
                  {w.status === "pending" && (
                    <>
                      <button
                        ...
                        className="px-4 py-2 bg-green-600 text-white rounded-xl text-xs"
                      >
                        Approve
                      </button>
                      <button
