...
const CategoryGrid: React.FC<CategoryGridProps> = ({ categories }) => {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6 p-2 md:p-4">
      {categories.map((cat) => {
        const imgUrl = getImageUrl(cat.image_url || cat.image);
        return (
          <Link
            key={cat.id}
          ...
          >
            {/* Image Section */}
            <div className="w-full aspect-[16/10] sm:aspect-auto sm:h-40 md:h-52 bg-gray-50 overflow-hidden relative">
              {imgUrl ? (
                <Image
                  src={imgUrl}
                  alt={cat.name}
            ...
            <div className="p-3 md:p-4">
              <h3 className="text-sm sm:text-base md:text-lg font-semibold text-gray-800 group-hover:text-blue-600 transition-colors line-clamp-1">
                {cat.name}
              </h3>
            </div>
          </Link>
